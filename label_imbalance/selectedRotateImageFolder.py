...
def _resolve_corruption_dir(data_corruption_root, corruption, level):
    """Resolve ImageNet-C corruption path in either flat or category-nested layout."""
    direct = os.path.join(data_corruption_root, corruption, str(level))
    if os.path.isdir(direct):
        return direct

    for entry in os.scandir(data_corruption_root):
        if not entry.is_dir():
            continue
        candidate = os.path.join(entry.path, corruption, str(level))
        if os.path.isdir(candidate):
            return candidate

    raise FileNotFoundError(
        f"Could not find corruption path for {corruption} level {level} under {data_corruption_root}"
    )
...
def prepare_test_data(args, use_transforms=True):	
    ...
    if not hasattr(args, 'corruption') or args.corruption == 'original':
        ...
        validdir = _resolve_corruption_dir(args.data_corruption, args.corruption, args.level)
        teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False,
                                          rotation=False,
                                                    rotation_transform=rotation_te_transforms)
    else:
