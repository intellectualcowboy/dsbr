...
def accuracy_ent(network, loader, weights, device, adapt=False, run=None, prev_acc=None, prev_ent=None):
    correct = 0
    total = 0
    weights_offset = 0
    ent = 0
        ...
        for x, y in tqdm(loader):
            ...
            if run is not None:
                run.log({'accuracy': correct / total,
                    'entropy': ent / total,
                    'delta_accuracy': (correct / total) - prev_acc,
                    'delta_entropy': (ent / total) - prev_ent
                })
    network.train()
...
if __name__ == "__main__":
    ...
    prev_acc, prev_ent = results[name+'_acc'], results[name+'_ent']
    for adapt_hparams in adapt_hparams_list:
        ...
        if args.wandb_project is not None:
            with wandb.init(project=args.wandb_project,
                        config={**vars(args), **hparams, **adapt_hparams},
                        tags=args_in.wandb_tags) as run:
                acc, ent = accuracy_ent(adapted_algorithm, loader, weights, device, adapt=True, run=run, prev_acc=prev_acc, prev_ent=prev_ent)
        else:
                acc, ent = accuracy_ent(adapted_algorithm, loader, weights, device, adapt=True)
        results[name+'_acc'] = acc
        results[name+'_ent'] = ent
