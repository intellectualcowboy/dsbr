...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if hparams.get("use_image_net_pretrained", False):
            model = networks.Featurizer(input_shape, self.hparams)
            self.featurizer = model.network
    ...
    def update(self, minibatches, unlabeled=None):
        steps = self.hparams.get('grad_accumulation_steps', 1)
        ...
        n = len(all_x)
        x_chunks = all_x.chunk(steps)
        y_chunks = all_y.chunk(steps)

        self.optimizer.zero_grad()
        total_loss = 0.0
        next_x = x_chunks[0].to(device, non_blocking=True)
        next_y = y_chunks[0].to(device, non_blocking=True)
        for i in range(len(x_chunks)):
            x_chunk, y_chunk = next_x, next_y
            # Weight by chunk fraction so accumulated gradient == full-batch gradient
            chunk_loss = F.cross_entropy(self.predict(x_chunk), y_chunk) * (len(x_chunk) / n)
            chunk_loss.backward()
            total_loss += chunk_loss.item()
            # prefetch after backward so the graph is freed before next chunk lands on GPU
            if i + 1 < len(x_chunks):
                next_x = x_chunks[i + 1].to(device, non_blocking=True)
                next_y = y_chunks[i + 1].to(device, non_blocking=True)
        self.optimizer.step()
