...
class IIDROCAUCSelectionMethod(SelectionMethod):
    """Picks argmax(mean(env_out_roc_auc for env in train_envs))"""
    name = "training-domain validation set (ROC AUC)"
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            if f'env{i}_out_roc_auc' not in record:
                break
            if i in test_envs:
                test_env_keys.append(f'env{i}_out_roc_auc')
            else:
                val_env_keys.append(f'env{i}_out_roc_auc')
        return {
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            if f'env{i}_out_acc' not in record:
                break
            if i in test_envs:
                test_env_keys.append(f'env{i}_out_acc')
            else:
