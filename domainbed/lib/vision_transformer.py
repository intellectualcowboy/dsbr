...
class ViT2(torch.nn.Module):
    ...
    KNOWN_MODELS_IN1K = {
        'ViT-B16': timm.models.vision_transformer.vit_base_patch16_224, 
        'ViT-B32': timm.models.vision_transformer.vit_base_patch32_224,
        'ViT-L16': timm.models.vision_transformer.vit_large_patch16_224,
        'ViT-L32': timm.models.vision_transformer.vit_large_patch32_224,
        'ViT-H14': timm.models.vision_transformer.vit_huge_patch14_224
    }

    def __init__(self, input_shape, hparams):
        super().__init__()
        if hparams.get("use_image_net_pretrained", False):
            func = self.KNOWN_MODELS_IN1K[hparams['backbone']]
            self.network = func(pretrained=True)
            self.n_outputs = self.network.head.out_features
            self.classifier = copy.deepcopy(self.network.head)
